...
const RESOURCE: ReaderResource = {
  id: "devops-answer-key",
  title: "DevOps — Answer Key",
  ...
  heroSubtitle: "Reasoned answers and explanations for the DevOps practice questions.",
};
...
export const Route = createFileRoute("/_marketing/resources/read/devops-answer-key")({
  head: () => ({
    meta: [
    ...
    links: [{ rel: "canonical", href: "/resources/read/devops-answer-key" }],
  }),
  component: Page,
});
  ...
  return (
    <ReaderShell resource={RESOURCE} toc={TOC} related={RELATED}>
      ...
      <Section id="intro" title="Introduction">
        <p>Reasoned answers and explanations for the DevOps practice questions. DevOps is a cultural and technical movement that unifies development and operations to deliver software safely, frequently, and reliably.</p>
...
deploy: kubectl set image deploy/app app=registry/app:$SHA`}</Code>
      ...
      <Section id="answers" title="Answer Explanations">
        <ul className="list-disc space-y-1 pl-5">
          <li><b>GitHub Actions PR tests</b> — trigger on pull_request, matrix Node versions, cache deps with actions/cache.</li>
          <li><b>Multi-stage Dockerfile</b> — build in node:20-alpine, copy artifacts into node:20-alpine runtime; drop dev deps.</li>
          <li><b>Terraform S3 module</b> — aws_s3_bucket + aws_s3_bucket_versioning resources; publish inputs/outputs.</li>
          <li><b>K8s Deployment</b> — replicas 3, liveness/readiness probes, resource requests/limits.</li>
          <li><b>Prometheus</b> — scrape /metrics, expose via prom-client, add a ServiceMonitor under prometheus-operator.</li>
          <li><b>Ansible nginx</b> — apt/yum + service modules; handlers for reload.</li>
          <li><b>Helm values</b> — one values.yaml per environment, override via -f values-prod.yaml.</li>
          <li><b>Log rotation</b> — find /var/log -mtime +7 -name *.log -delete, wrap in cron.</li>
          <li><b>Canary rollout</b> — Argo Rollouts strategy.canary.steps with pauses and traffic weights.</li>
          <li><b>HPA</b> — autoscaling/v2 HorizontalPodAutoscaler with CPU target 70%.</li>
        </ul>
      </Section>

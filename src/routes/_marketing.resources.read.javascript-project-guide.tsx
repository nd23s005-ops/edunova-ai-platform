...
const RESOURCE: ReaderResource = {
  ...
  heroSubtitle: "How to build production-ready JavaScript apps \u2014 planning, architecture, API integration, testing, CI/CD, and deployment.",
};
...
const TOC: TocItem[] = [
  ...
  { id: "s2", label: "2. Folder Structure" },
  { id: "s3", label: "3. Architecture" },
  { id: "s4", label: "4. API Integration" },
  { id: "s5", label: "5. Testing" },
  { id: "s6", label: "6. CI/CD" },
  { id: "s7", label: "7. Documentation" },
  { id: "s8", label: "8. Deployment" },
...
export const Route = createFileRoute("/_marketing/resources/read/javascript-project-guide")({
  head: () => ({
    meta: [
    ...
    links: [{ rel: "canonical", href: "/resources/read/javascript-project-guide" }],
  }),
  component: Page,
});
    ...
    <ReaderShell resource={RESOURCE} toc={TOC} related={RELATED}>
      <Section id="objectives" title="Learning Objectives">
        <ul className="list-disc space-y-1 pl-5">
          <li>Understand the core concepts covered in JavaScript — Project Guide.</li>
      ...
      <Section id="intro" title="Introduction">
        <p>A framework-agnostic playbook for shipping a JavaScript project from idea to production.</p>
      ...
      <Section id="s1" title="1. Planning">
        <p>User stories, wireframes, tech stack.</p>
        <Code lang="js">{`// Planning — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from planning";
};`}</Code>
      </Section>

      <Section id="s2" title="2. Folder Structure">
        <p>src/, tests/, config, features.</p>
        <Code lang="js">{`// Folder Structure — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from folder structure";
};`}</Code>
      </Section>

      <Section id="s3" title="3. Architecture">
        <p>Layered modules, dependency injection.</p>
        <Code lang="js">{`// Architecture — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from architecture";
};`}</Code>
      </Section>

      <Section id="s4" title="4. API Integration">
        <p>fetch wrappers, error handling.</p>
        <Code lang="js">{`// API Integration — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from api integration";
};`}</Code>
      </Section>

      <Section id="s5" title="5. Testing">
        <p>Unit (Vitest), integration (Playwright).</p>
...
const example = () => {
  ...
  return "hello from testing";
      ...
      <Section id="s6" title="6. CI/CD">
        <p>GitHub Actions, preview deploys.</p>
        <Code lang="js">{`// CI/CD — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from ci/cd";
      ...
      <Section id="s7" title="7. Documentation">
        <p>README, ADRs, JSDoc.</p>
        <Code lang="js">{`// Documentation — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from documentation";
};`}</Code>
      </Section>

      <Section id="s8" title="8. Deployment">
        <p>Vercel/Netlify/Cloudflare.</p>
...
const example = () => {
  ...
  return "hello from deployment";
      ...
      <Section id="s9" title="9. Maintenance">
        <p>Monitoring, dependency updates.</p>
        <Code lang="js">{`// Maintenance — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from maintenance";
};`}</Code>
      </Section>
...
Next Macrotask (setTimeout, I/O, UI events)`}</Code>
      ...
      <Section id="summary" title="Summary & Key Takeaways">
        <p>JavaScript — Project Guide equips you with the practical knowledge to write, debug, and ship JavaScript with confidence.</p>
        <ul className="list-disc space-y-1 pl-5">

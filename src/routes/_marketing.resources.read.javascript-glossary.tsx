...
const RESOURCE: ReaderResource = {
  ...
  heroSubtitle: "Alphabetical glossary \u2014 JS terminology, ES2020+, async, browser APIs, DOM, patterns, interview terms.",
};
...
export const Route = createFileRoute("/_marketing/resources/read/javascript-glossary")({
  head: () => ({
    meta: [
    ...
    links: [{ rel: "canonical", href: "/resources/read/javascript-glossary" }],
  }),
  component: Page,
});
    ...
    <ReaderShell resource={RESOURCE} toc={TOC} related={RELATED}>
      <Section id="objectives" title="Learning Objectives">
        <ul className="list-disc space-y-1 pl-5">
          <li>Understand the core concepts covered in JavaScript — Glossary.</li>
      ...
      <Section id="intro" title="Introduction">
        <p>Every term you'll hear in modern JavaScript, defined in plain language.</p>
      ...
      <Section id="s1" title="1. A — E">
        <p>Arrow fn, Async, Callback, Closure, Event Loop.</p>
        <Code lang="js">{`// A — E — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from a — e";
};`}</Code>
      </Section>

      <Section id="s2" title="2. F — J">
        <p>Function, Generator, Hoisting, IIFE, JSON.</p>
        <Code lang="js">{`// F — J — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from f — j";
};`}</Code>
      </Section>

      <Section id="s3" title="3. K — O">
        <p>Lexical scope, Microtask, Module, Nullish, Object.</p>
        <Code lang="js">{`// K — O — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from k — o";
};`}</Code>
      </Section>

      <Section id="s4" title="4. P — T">
        <p>Promise, Prototype, Rest/Spread, Symbol, TDZ.</p>
        <Code lang="js">{`// P — T — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from p — t";
};`}</Code>
      </Section>

      <Section id="s5" title="5. U — Z">
        <p>Undefined vs Null, WeakMap, Yield.</p>
        <Code lang="js">{`// U — Z — illustrative snippet
const example = () => {
  // apply concepts covered above
  return "hello from u — z";
};`}</Code>
      </Section>
...
Next Macrotask (setTimeout, I/O, UI events)`}</Code>
      ...
      <Section id="summary" title="Summary & Key Takeaways">
        <p>JavaScript — Glossary equips you with the practical knowledge to write, debug, and ship JavaScript with confidence.</p>
        <ul className="list-disc space-y-1 pl-5">
